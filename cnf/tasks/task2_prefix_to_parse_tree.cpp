...
Node* buildTreeFromPrefixTokens(const vector<string>& tokens, size_t& index) {
    ...
    if (token == "~")
        return negationNode(buildTreeFromPrefixTokens(tokens, index));
    else if (token == "+")
        return disjunctionNode(buildTreeFromPrefixTokens(tokens, index),
                           buildTreeFromPrefixTokens(tokens, index));
    else if (token == "*")
        return conjunctionNode(buildTreeFromPrefixTokens(tokens, index),
                           buildTreeFromPrefixTokens(tokens, index));
    else if (token[0] == 'x')
