...
bool applynegationNode(double negProb) {
...
std::string generateFormulaRec(int depth, int minDepth, int maxDepth,
    ...
    if (depth >= maxDepth || (depth >= minDepth && rand() % 2 == 0)) {
        std::string lit = "";
        if (applynegationNode(negProb)) lit += "~";
...
std::vector<std::string> generateRandomFormulas(int numFormulas,
    ...
    for (int i = 0; i < numFormulas; ++i) {
        formulas.push_back(
            generateFormulaRec(0, minDepth, maxDepth, atoms, negationNodeProb));
    }
    return formulas;
}